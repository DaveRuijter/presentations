...
dbutils.widgets.text("pbi_group_id", "default")
dbutils.widgets.text("pbi_dataset_id", "default")
dbutils.widgets.text("pbi_checkpoint_name", "validate_pbi_orders_by_month")
dbutils.widgets.text("pbi_query", """
EVALUATE
SUMMARIZECOLUMNS(
    'Date'[Year],
    'Date'[MonthNameShort],
...
def get_notebook_widgets_values() -> Dict:
    widgets_values = {
        "pbi_group_id": dbutils.widgets.get("pbi_group_id"),
        "pbi_dataset_id": dbutils.widgets.get("pbi_dataset_id"),
        "pbi_checkpoint_name": dbutils.widgets.get("pbi_checkpoint_name"),
        "pbi_query": dbutils.widgets.get("pbi_query")
    }
...
# MAGIC %run ./71_get_powerbi_df

# COMMAND ----------

# DBTITLE 1,Run checkpoint (execute data validation)
checkpoint_name=f"{widgets_values['pbi_checkpoint_name']}"
